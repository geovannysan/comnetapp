...
const Conciliacion = () => {
    ...
    const [fileUploadedbprod, setfileUploadedbprod] = useState(false)
    const [transacion, setTransacion] = useState([]);
    const [archivos, setArchivos] = useState([]);
    const [archivoProd, setProdubanco] = useState([]);
    ...
    const Subir_archivo = async () => {
        ...
        setTimeout(function () {
            const totalItems = [...archivos, ...archivoProd, ...archivoGua, ...archivosPa].length;
                ...
                const nuevos = transacion.map((e, index) => {
                    ...
                    if (String(e["Forma de Pago"]).includes("PRODUBANCO")) {
                        ...
                        if (objetoCoincidente) {
                            ...
                            ) ? `Valor Correcto ${objetoCoincidente.VALOR}` : `Valor incorrecto ${objetoCoincidente.VALOR}`;

                            e.estado = "ok PRODUBANCO";
                            return e;
                        }
                    }
    ...
    const handleFileUploadbancoProdu = (event) => {
        // Verificar si se cargó un archivo
        if (event.target.files.length > 0) {
            // Cambiar el estado para indicar que se ha cargado un archivo
            setfileUploadedbprod(true);
            const reader = new FileReader();
            reader.onload = (e) => {
                const data = e.target.result;
                const workbook = xlsx.read(data, { type: "array" });
                ...
                const json = xlsx.utils.sheet_to_json(worksheet);
                setProdubanco(json)
                console.log(json.filter(ele => ele.REFERENCIA == '69246311'));
            };
            reader.readAsArrayBuffer(event.target.files[0]);
        } else {
            setArchivos([])
            ...
            <div className="p-3">
                ...
                <div className=" d-flex  justify-content-around align-items-end flex-wrap">
                    ...
                    <label className="custum-file-upload2"  >
                        ...
                        <div className="text">
                            {fileUploadedban ? (<span id="banco"></span>) : (<span>Banco Pichincha</span>)}
                        </div>
                    ...
                    <label className="custum-file-upload2"  >
                        ...
                        <div className="text">
                            {fileUploadedbprod ? (<span id="banco"></span>) : (<span>Banco PRODUBANCO</span>)}
                        </div>
                        <input type="file" id="banco" onChange={handleFileUploadbancoProdu} accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" />
                ...
                <div className="d-flex justify-content-center pt-2">
                    {([...archivoGua, ...archivos, ...archivosPa,...archivoProd].length > 0) ? <button className="btn btn-primary" onClick={Subir_archivo}>
                        Comparar
                    </button> : ""}
                </div>
