            ...
            return transacion.map((item, index) => {
                ...
                return (
                    <tr key={index}>
                        ...
                        <td className={item["estado"] == "OK" ? "bg-danger font-weight-bold" : "font-weight-bold"}>
                            {item["estado"] || " "}
                        </td>
                        <td className=" font-weight-bold">
                            {item["Operador"]}
    ...
    const Subir_archivo = async () => {
        if (archivos.length > 0 && transacion.length > 0) {
            ...
            let nuevos = transacion.map(e => {
                if (archivos.some(item => Object.values(item).some(value => String(value).includes(e["# Transacción"])))) {
                    console.log(`El texto "${e["# Transacción"]}" está presente en algún valor de la transacción.`);
                    e.estado = "OK"
                    return e;
                } else {
                    console.log(`El texto "${e["# Transacción"]}" no está presente en ningún valor de la transacción.`);
                    return e
                }
                return e
            })
            ...
            <div className="p-3">
                ...
                <div className='d-flex  mt-3 pt-3 mb-1  pb-3 text-center  justify-content-around '>
                    ...
                    <label className="custum-file-upload" >
                        ...
                        <div className="text">
                            {fileUploadedban ? (<span></span>) : (<span>Cargar de banco</span>)}
                        </div>
